...
|---|---|---|---|
| `IA` | intervalo entre arribos | normal truncada positiva, media 360 y desvío 126 | min; media 6 horas y desvío 2,1 horas |
| `CantLotes` | lotes por pedido | normal discreta truncada, media 5 y desvío 1,5 | entero positivo |
...
`CostoPromLote = CostoTotal / CTLFin`, si `CTLFin > 0`.

Estas expresiones distribuyen los costos modelados sobre los pedidos o lotes del alcance temporal de la corrida. La mano de obra incluida corresponde exclusivamente a cambios de configuración; no se incorporan despacho, almacenamiento, otra mano de obra, merma fuera del reproceso ni capital inmovilizado.
...
> **[Lugar previsto para Figura 4 — Política de franja cara.]** Incorporar una línea de tiempo de 24 horas con `InicioCaro`, `FinCaro` y dos ramas: continuar (`PEFC = verdadero`) o pausar toda preparación y producción durante la ventana (`PEFC = falso`). Señalar el costo o la espera agregada en cada rama.
